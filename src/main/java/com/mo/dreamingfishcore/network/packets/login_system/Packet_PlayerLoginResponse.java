...
import com.mo.dreamingfishcore.core.login_system.PlayerLoginData;
import com.mo.dreamingfishcore.core.login_system.PlayerLoginDataManager;
import com.mo.dreamingfishcore.core.playerattributes_system.death.RevivalInfoManager;
import com.mo.dreamingfishcore.server.notice.NewPlayerGuide;
...
public class Packet_PlayerLoginResponse implements net.minecraft.network.protocol.common.custom.CustomPacketPayload {
    ...
    public static void handle(Packet_PlayerLoginResponse playerLoginResponse, IPayloadContext context) {
        com.mo.dreamingfishcore.EconomySystem.LOGGER.info("收到密码响应包！");

        if (playerLoginResponse == null) {
            com.mo.dreamingfishcore.EconomySystem.LOGGER.error("密码响应包为null！");
            return;
        }
        ServerPlayer serverPlayer = context.player() instanceof ServerPlayer player ? player : null;

        if (serverPlayer == null) {
            com.mo.dreamingfishcore.EconomySystem.LOGGER.error("服务端玩家实例为null！");
            return;
        }

        com.mo.dreamingfishcore.EconomySystem.LOGGER.info("玩家 {} 发送密码响应，loginOrRegister={}",
    ...
    private static void sendResult(ServerPlayer player, boolean success, String message) {
        com.mo.dreamingfishcore.network.EconomySystem_NetworkManager.sendToClient(
