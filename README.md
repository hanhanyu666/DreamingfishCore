...
DreamingfishCore 是面向梦鱼服的 Minecraft NeoForge 1.21.1 核心模组，提供服务器玩法、角色成长、终端界面、剧情任务、蓝图合成限制和基础管理能力。它更像服务器的底层玩法框架：负责把玩家数据、服务器 UI、世界规则和自定义物品串在一起，让服务器可以在原版体验之上拥有更完整的长期游玩结构。
...
- 自定义内容：包含药品、虫洞药水、回忆药水、复活护符、启程锦鲤、赞助者帽子、蜂巢僵尸、声音与附魔。
- 传送请求：提供基于虫洞药水的 TPA 请求、接受、拒绝与超时流程。
